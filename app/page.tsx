...
const records: Record<string, Row[]> = {
  ...
  Opportunities: [
    ...
    { id:"OPP-281", name:"Annual renewal", account:"Mira Systems", owner:"Sarah Chen", value:"$64,200", status:"Secured", date:"Aug 18, 2026" },
  ],
  Quotations: [
...
function Overview({navigate,announce}:{navigate:(label:string)=>void,announce:(message:string)=>void}) {
  const opportunityStatusByMonth:Record<string,{status:string,count:number}[]>={
    Mar:[{status:"Bidding",count:8},{status:"Proposal",count:5},{status:"Negotiation",count:3},{status:"Secured",count:2},{status:"Delivery Status",count:1}],
    Apr:[{status:"Bidding",count:10},{status:"Proposal",count:6},{status:"Negotiation",count:4},{status:"Secured",count:3},{status:"Delivery Status",count:2}],
    May:[{status:"Bidding",count:12},{status:"Proposal",count:8},{status:"Negotiation",count:5},{status:"Secured",count:4},{status:"Delivery Status",count:2}],
    Jun:[{status:"Bidding",count:11},{status:"Proposal",count:9},{status:"Negotiation",count:6},{status:"Secured",count:5},{status:"Delivery Status",count:3}],
    Jul:[{status:"Bidding",count:14},{status:"Proposal",count:10},{status:"Negotiation",count:7},{status:"Secured",count:6},{status:"Delivery Status",count:4}],
    Aug:[{status:"Bidding",count:16},{status:"Proposal",count:12},{status:"Negotiation",count:9},{status:"Secured",count:7},{status:"Delivery Status",count:5}],
  };
...
function RecordsPage({module,query,filter,setFilter,announce,onNewContact,onEditCustomer,onDeleteCommercial,onDeleteContact,onDeleteCustomer,onDeleteEmployee,rowsOverride,sourceCountOverride}:{module:string,query:string,filter:string,setFilter:(x:string)=>void,announce:(x:string)=>void,onNewContact:()=>void,onEditCustomer:(customer:Row)=>void,onDeleteCommercial:(record:Row)=>void,onDeleteContact:(contact:Row)=>void,onDeleteCustomer:(customer:Row)=>void,onDeleteEmployee:(employee:Row)=>void,rowsOverride?:Row[],sourceCountOverride?:number}) {
  ...
  const flowStages=module==="Opportunities"?["All statuses","Bidding","Proposal","Negotiation","Secured","Delivery Status"]:[];
...
function ReportDetail({name,range}:{name:string,range:string}) {
  const rows:Record<string,string[][]>={
    ...
    "Opportunity conversion":[["Bidding","42","$820k","100%"],["Proposal","24","$408k","57%"],["Negotiation","13","$264k","31%"],["Secured","8","$184k","19%"]],
  };
