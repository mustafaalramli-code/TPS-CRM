...
export default function Home() {
  ...
  useEffect(()=>{if(active==="Opportunities"&&!['Inquiry Information','History'].includes(opportunitySection))setOpportunitySection("Inquiry Information")},[active,opportunitySection]);
  ...
  function saveDrawer(e: any) {
    ...
    if (active === "Opportunities" || active === "Quotations") {
      ...
      const commercial:Row={id:editingCustomer?.id||`${active==="Quotations"?"QUO":"OPP"}-${String(286+targetRows.length).padStart(3,"0")}`,name:recordName,account:control("Client")||control("End User")||"Unassigned client",owner:control("Employee")||"Alex Morgan",value:control("TPS Offer")||control("Supplier Total")||control("Offer Value")||"To be valued",status:control("Client Inquiry Status")||control("Project Status")||"Qualified",date:control("Offer Date")||new Date().toLocaleDateString()};
      ...
      <aside className="drawer access-drawer" role="dialog" aria-modal="true" aria-label={`${editingCustomer?"Edit":"Add"} ${actionName}`} onMouseDown={e => e.stopPropagation()}>
        ...
        <form key={`${editingCustomer?.id||"new"}-${customerPage}`} className={active === "Customers" ? "has-tabs" : active==="Opportunities"?"has-opportunity-tabs":""} onSubmit={saveDrawer}>
          {active==="Opportunities"?<>
            <fieldset className="opportunity-reference-summary"><legend>Opportunity details</legend><div className="drawer-field-grid"><label>Client<select defaultValue={editingCustomer?.account||""}><option value="">Select...</option>{customerRows.map(client=><option key={client.id}>{client.name}</option>)}</select></label><label>Opportunity Type<select defaultValue=""><option value="">Select...</option>{selectValues("Opportunity Type").map(value=><option key={value}>{value}</option>)}</select></label><label>Project Name<input defaultValue={editingCustomer?.name||""}/></label><label>Client Inquiry Status<select defaultValue={editingCustomer?.status||"On Hand"}><option>On Hand</option><option>Qualified</option><option>Proposal</option><option>Negotiation</option><option>Won</option></select></label><label>Document Type<select defaultValue="Offer"><option>Inquiry</option><option>Offer</option><option>Order</option><option>Secured</option></select></label><label>Employee<select defaultValue={editingCustomer?.owner||employeeRows[0]?.name||""}>{employeeRows.map(employee=><option key={employee.id}>{employee.name}</option>)}</select></label><label>Year<input type="number" defaultValue={new Date().getFullYear()}/></label></div></fieldset>
