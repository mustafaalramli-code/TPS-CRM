...
const records: Record<string, Row[]> = {
  ...
  Quotations: [
    { id:"QUO-631", name:"Northstar phase II", account:"Northstar Labs", owner:"Sarah Chen", value:"$184,000", status:"Delivered", date:"Aug 1, 2026" },
  ],
  Projects: [
...
export default function Home() {
  ...
  const [customerOpportunityRows,setCustomerOpportunityRows]=useState<CustomerOpportunity[]>([
    ...
    {customerId:"CUS-1047",id:"OPP-259",name:"Equipment upgrade",type:"New",value:"$88,750",stage:"Bidding",owner:"Alex Morgan",close:"2026-10-04"},
  ]);
  ...
  const drawerGroups: any = active === "Customers" && customerPage === "Opportunities" ? {map:()=><CustomerQuotationList customer={editingCustomer} contacts={contactListRows} announce={announce} employees={employeeRows} items={opportunityRows} setItems={setOpportunityRows} onWon={project=>setWonProjects(rows=>rows.some(row=>row.id===project.id)?rows:[project,...rows])} onOpenWorkspaceOpportunity={opportunity=>{setOpportunityClientContext(opportunity?.account||editingCustomer?.name||"");setActive("Opportunities");setEditingCustomer(opportunity);setOpportunitySection("Inquiry Information");setDrawer(true)}}/>} : active === "Customers" && customerPage === "Suppliers" ? {map:()=><CustomerSuppliers announce={announce}/>} : active === "Customers" && customerPage === "Projects" ? {map:()=><CustomerProjects customer={editingCustomer} announce={announce} wonProjects={wonProjects} workspaceProjects={records.Projects}/>} : active === "Customers" && customerPage === "Activities" ? {map:()=><CustomerActivities announce={announce}/>} : active === "Customers" && customerPage === "Delivery" ? {map:()=><CustomerDeliveries announce={announce}/>} : active === "Customers" && customerPage === "Tasks" ? {map:()=><CustomerTasks announce={announce}/>} : active === "Customers" ? customerPages.find(page => page.name === customerPage)?.groups : active === "Projects" && editingCustomer ? {map:()=><CustomerProjects customer={customerRows.find(client=>client.name===editingCustomer.account)||null} announce={announce} wonProjects={wonProjects} workspaceProjects={records.Projects} initialProject={editingCustomer}/>} : activeForm?.groups;
        ...
        <form key={`${editingCustomer?.id||"new"}-${customerPage}`} className={active === "Customers" ? "has-tabs" : active==="Opportunities"?"has-opportunity-tabs":""} onSubmit={saveDrawer}>
          {active==="Opportunities"?<>
            <fieldset className="opportunity-reference-summary"><legend>Opportunity details</legend><div className="drawer-field-grid">
              ...
              <label>Opportunity Type<select defaultValue=""><option value="">Select...</option>{selectValues("Opportunity Type").map(value=><option key={value}>{value}</option>)}</select></label><label>Project Name<input defaultValue={editingCustomer?.name||""}/></label><label>Client Inquiry Status<select defaultValue={editingCustomer?.status||"On Hand"}><option>On Hand</option><option>Bidding</option><option>Proposal</option><option>Negotiation</option><option>Secured</option></select></label><label>Document Type<select defaultValue="Offer"><option>Inquiry</option><option>Offer</option><option>Order</option><option>Secured</option></select></label><label>Employee<select defaultValue={editingCustomer?.owner||employeeRows[0]?.name||""}>{employeeRows.map(employee=><option key={employee.id}>{employee.name}</option>)}</select></label><label>Year<input type="number" defaultValue={new Date().getFullYear()}/></label>
            </div></fieldset>
...
function CustomerQuotationList({customer,contacts,announce,employees,items,setItems,onWon,onOpenWorkspaceOpportunity}:{customer:Row|null,contacts:Row[],announce:(message:string)=>void,employees:Row[],items:Row[],setItems:Dispatch<SetStateAction<Row[]>>,onWon:(project:WonProject)=>void,onOpenWorkspaceOpportunity:(opportunity:Row|null)=>void}) {
  ...
  const save=()=>{const name=nameRef.current?.value.trim();if(!name){announce("Project Name is required");return;}const statusLabel=Array.from(document.querySelectorAll("label")).find(label=>label.childNodes[0]?.textContent?.trim()==="Client Inquiry Status");const status=(statusLabel?.querySelector("select") as HTMLSelectElement|null)?.value||selected?.status||"On Hand";const item:Row={id:selected?.id||`OPP-${285+items.length}`,name,account:customer?.name||"",value:valueRef.current?.value.trim()||selected?.value||"$0",status,owner:ownerRef.current?.value||selected?.owner||"Alex Morgan",date:new Date().toLocaleDateString()};const note=noteRef.current?.value.trim()||"";const historyEntry=`${new Date().toLocaleString()} — ${selected?"Updated":"Created"}: ${item.name}; status ${item.status}; value ${item.value}; handled by ${item.owner}${note?`; note: ${note}`:""}`;setHistoryById(history=>({...history,[item.id]:[historyEntry,...(history[item.id]||[])]}));if(status==="Secured"){onWon({id:item.id.replace(/^QUO|^OPP/,"PRJ"),customerId:customer?.id||"",name:item.name,type:"Opportunity conversion",status:"New",progress:"0%",start:new Date().toLocaleDateString(),due:"To be scheduled"});setItems(rows=>rows.filter(row=>row.id!==item.id));setSelected(null);setIsNew(false);announce("Secured opportunity transferred to the Project list");return;}setItems(rows=>selected?rows.map(row=>row.id===selected.id?item:row):[item,...rows]);setSelected(null);setIsNew(false);announce(note?"Opportunity saved and note moved to History":"Opportunity saved in the client and workspace lists")};
...
function Overview({navigate,announce}:{navigate:(label:string)=>void,announce:(message:string)=>void}) {
  const opportunityStatusByMonth:Record<string,{status:string,count:number}[]>={
    Mar:[{status:"Bidding",count:8},{status:"Proposal",count:5},{status:"Negotiation",count:3},{status:"Ready for delivery",count:2},{status:"Delivered",count:1}],
    Apr:[{status:"Bidding",count:10},{status:"Proposal",count:6},{status:"Negotiation",count:4},{status:"Ready for delivery",count:3},{status:"Delivered",count:2}],
    May:[{status:"Bidding",count:12},{status:"Proposal",count:8},{status:"Negotiation",count:5},{status:"Ready for delivery",count:4},{status:"Delivered",count:2}],
    Jun:[{status:"Bidding",count:11},{status:"Proposal",count:9},{status:"Negotiation",count:6},{status:"Ready for delivery",count:5},{status:"Delivered",count:3}],
    Jul:[{status:"Bidding",count:14},{status:"Proposal",count:10},{status:"Negotiation",count:7},{status:"Ready for delivery",count:6},{status:"Delivered",count:4}],
    Aug:[{status:"Bidding",count:16},{status:"Proposal",count:12},{status:"Negotiation",count:9},{status:"Ready for delivery",count:7},{status:"Delivered",count:5}],
  };
  const [opportunityMonth,setOpportunityMonth]=useState("Aug");
  const statusRows=opportunityStatusByMonth[opportunityMonth];
  const maxStatusCount=Math.max(...statusRows.map(row=>row.count));
  return <>
  ...
  <div className="main-grid"><article className="panel revenue opportunity-status-report"><div className="panel-head"><div><h2>Opportunity status report</h2><p>Pipeline status filtered by the selected month</p></div><strong>{opportunityMonth}</strong></div><div className="opportunity-month-filter" aria-label="Select report month">{Object.keys(opportunityStatusByMonth).map(month=><button type="button" key={month} className={month===opportunityMonth?"active":""} onClick={()=>{setOpportunityMonth(month);announce(`Opportunity status report filtered by ${month}`)}}>{month}</button>)}</div><div className="status-report-bars" aria-label={`Opportunity status for ${opportunityMonth}`}>{statusRows.map(row=><div className="status-report-row" key={row.status}><span>{row.status}</span><div><i style={{width:`${Math.max(8,row.count/maxStatusCount*100)}%`}}/></div><strong>{row.count}</strong></div>)}</div></article>
