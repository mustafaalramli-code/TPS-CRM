  ...
  return <main className="app-shell">
    ...
    {drawer && <div className={`drawer-backdrop ${editingCustomer&&active==="Customers"?"customer-fullpage":""}`} onMouseDown={() => setDrawer(false)}>
      <aside className="drawer access-drawer" role="dialog" aria-modal="true" aria-label={`${editingCustomer?"Edit":"Add"} ${actionName}`} onMouseDown={e => e.stopPropagation()}>
        ...
        {active === "Opportunities" && <nav className="opportunity-section-tabs" aria-label="Opportunity form sections">{activeForm?.groups.map(([title]:[string,string[]])=><button type="button" key={title} className={opportunitySection===title?"active":""} onClick={()=>setOpportunitySection(title)}>{title}</button>)}</nav>}
