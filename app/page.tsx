...
function CustomerQuotationList({customer,announce,employees,items,setItems,onWon}:{customer:Row|null,announce:(message:string)=>void,employees:Row[],items:Row[],setItems:Dispatch<SetStateAction<Row[]>>,onWon:(project:WonProject)=>void}) {
  ...
  const primaryContact=clientContactDetails.find(contact=>contact.client===customer?.name);
  ...
  if(selected||isNew)return <section className="contact-detail opportunity-access"><div className="related-head"><div><button type="button" className="back-link" onClick={()=>{setSelected(null);setIsNew(false)}}>&lt;- Opportunity list</button><h3>{isNew?"New opportunity":selected?.name}</h3></div><button type="button" onClick={save}>Save Opportunities</button></div>
    ...
    <fieldset><legend>Client</legend><div className="drawer-field-grid"><label>Client<input value={customer?.name||selected?.account||""} readOnly/></label><label>Contact Person<input value={primaryContact?.name||""} readOnly/></label><label>Telephone<input value={primaryContact?.phone||""} readOnly/></label><label>Region<input value={primaryContact?.region||""} readOnly/></label><label>Location<input value={primaryContact?.location||""} readOnly/></label><label>Consultant<input/></label><label>End User<input value={customer?.name||selected?.account||""} readOnly/></label><label>Client Inquiry Status<select defaultValue={selected?.status||"On Hand"}><option>On Hand</option><option>Qualified</option><option>Proposal</option><option>Negotiation</option><option>Won</option></select></label></div></fieldset>
