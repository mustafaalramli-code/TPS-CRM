...
function CustomerQuotationList({customer,contacts,announce,employees,items,setItems,onWon}:{customer:Row|null,contacts:Row[],announce:(message:string)=>void,employees:Row[],items:Row[],setItems:Dispatch<SetStateAction<Row[]>>,onWon:(project:WonProject)=>void}) {
  ...
  return <section className="related-list"><div className="related-head"><div><h3>Opportunities</h3><p>{customerItems.length} opportunities filtered for {customer?.name||"this client"}</p></div><button type="button" onClick={()=>setIsNew(true)}>+ New opportunity</button></div><div className="table-scroll"><table><thead><tr><th>ID</th><th>Opportunity</th><th>Client / contact</th><th>Owner</th><th>Value</th><th>Status</th><th>Close date</th><th>Actions</th></tr></thead><tbody>{customerItems.map(item=><tr key={item.id} className="record-row" onClick={()=>setSelected(item)}><td><span className="record-id">{item.id}</span></td><td><strong>{item.name}</strong></td><td>{item.account}</td><td>{item.owner}</td><td>{item.value}</td><td><Status value={item.status}/></td><td>{item.date}</td><td><div className="row-actions"><button type="button" className="open-record" onClick={e=>{e.stopPropagation();setSelected(item)}}>Edit</button><button type="button" className="delete-record" onClick={e=>{e.stopPropagation();if(window.confirm(`Delete ${item.name}?`)){setItems(rows=>rows.filter(row=>row.id!==item.id));announce("Opportunity deleted")}}}>Del</button></div></td></tr>)}</tbody></table></div></section>;
}
