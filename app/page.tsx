...
export default function Home() {
  ...
  const [opportunitySection,setOpportunitySection]=useState("Inquiry");
  ...
  return <main className="app-shell">
    ...
    {drawer && <div className={`drawer-backdrop ${editingCustomer&&active==="Customers"?"customer-fullpage":""}`} onMouseDown={() => setDrawer(false)}>
      <aside className="drawer access-drawer" role="dialog" aria-modal="true" aria-label={`${editingCustomer?"Edit":"Add"} ${actionName}`} onMouseDown={e => e.stopPropagation()}>
        ...
        {active === "Opportunities" && <nav className="opportunity-section-tabs" aria-label="Opportunity form pages">{activeForm?.groups.map(group=>{const title=String(group[0]);return <button type="button" key={title} className={opportunitySection===title?"active":""} aria-selected={opportunitySection===title} onClick={()=>setOpportunitySection(title)}>{title}</button>})}</nav>}
