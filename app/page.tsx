...
export default function Home() {
  ...
  const drawerGroups: any = active === "Customers" && customerPage === "Opportunities" ? {map:()=><CustomerQuotationList customer={editingCustomer} contacts={contactListRows} announce={announce} employees={employeeRows} items={opportunityRows} setItems={setOpportunityRows} onWon={project=>setWonProjects(rows=>rows.some(row=>row.id===project.id)?rows:[project,...rows])} onOpenWorkspaceOpportunity={opportunity=>{setOpportunityClientContext(opportunity?.account||editingCustomer?.name||"");setActive("Opportunities");setEditingCustomer(opportunity);setOpportunitySection("Inquiry Information");setDrawer(true)}}/>} : active === "Customers" && customerPage === "Suppliers" ? {map:()=><CustomerSuppliers announce={announce}/>} : active === "Customers" && customerPage === "Projects" ? {map:()=><CustomerProjects customer={editingCustomer} announce={announce} wonProjects={wonProjects}/>} : active === "Customers" && customerPage === "Activities" ? {map:()=><CustomerActivities announce={announce}/>} : active === "Customers" && customerPage === "Delivery" ? {map:()=><CustomerDeliveries announce={announce}/>} : active === "Customers" && customerPage === "Tasks" ? {map:()=><CustomerTasks announce={announce}/>} : active === "Customers" ? customerPages.find(page => page.name === customerPage)?.groups : activeForm?.groups;
...
function RecordsPage({module,query,filter,setFilter,announce,onNewContact,onEditCustomer,onDeleteCommercial,onDeleteContact,onDeleteCustomer,onDeleteEmployee,rowsOverride,sourceCountOverride}:{module:string,query:string,filter:string,setFilter:(x:string)=>void,announce:(x:string)=>void,onNewContact:()=>void,onEditCustomer:(customer:Row)=>void,onDeleteCommercial:(record:Row)=>void,onDeleteContact:(contact:Row)=>void,onDeleteCustomer:(customer:Row)=>void,onDeleteEmployee:(employee:Row)=>void,rowsOverride?:Row[],sourceCountOverride?:number}) {
  ...
  const flowStages=module==="Opportunities"?["All statuses","Bidding","Proposal","Negotiation","Ready for delivery","Delivered"]:[];
