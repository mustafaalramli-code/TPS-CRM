...
type CustomerQuotation = { id:string; customerId:string; name:string; value:string; status:string; owner:string; date:string };
...
export default function Home() {
  ...
  const [customerQuotationRows,setCustomerQuotationRows]=useState<CustomerQuotation[]>([
  ...
  return <main className="app-shell">
    <header className="topbar">
      ...
      <button className="icon-btn" aria-label="Help" onClick={()=>announce("Help center opened")}>?</button><button className="icon-btn notification" aria-label="Notifications" onClick={()=>announce("No new notifications")}>N<i /></button><button className="avatar" onClick={()=>announce("Alex Morgan profile opened")}>AM</button>
    </header>
...
function CustomerQuotationList({customer,announce,employees,items,setItems,onWon}:{customer:Row|null,announce:(message:string)=>void,employees:Row[],items:CustomerQuotation[],setItems:Dispatch<SetStateAction<CustomerQuotation[]>>,onWon:(project:WonProject)=>void}) {
  const [selected,setSelected]=useState<CustomerQuotation|null>(null);const [isNew,setIsNew]=useState(false);const [detailView,setDetailView]=useState<"Inquiry Information"|"History">("Inquiry Information");const [historyById,setHistoryById]=useState<Record<string,string[]>>({});const nameRef=useRef<HTMLInputElement>(null);const valueRef=useRef<HTMLInputElement>(null);const ownerRef=useRef<HTMLSelectElement>(null);const noteRef=useRef<HTMLTextAreaElement>(null);
  const customerItems=items.filter(item=>item.customerId===customer?.id);
  const save=()=>{const name=nameRef.current?.value.trim();if(!name){announce("Project Name is required");return;}const statusLabel=Array.from(document.querySelectorAll("label")).find(label=>label.childNodes[0]?.textContent?.trim()==="Client Inquiry Status");const status=(statusLabel?.querySelector("select") as HTMLSelectElement|null)?.value||selected?.status||"On Hand";const item:CustomerQuotation={id:selected?.id||`OPP-${632+items.length}`,customerId:customer?.id||"",name,value:valueRef.current?.value.trim()||selected?.value||"$0",status,owner:ownerRef.current?.value||selected?.owner||"Alex Morgan",date:new Date().toLocaleDateString()};const note=noteRef.current?.value.trim()||"";const historyEntry=`${new Date().toLocaleString()} — ${selected?"Updated":"Created"}: ${item.name}; status ${item.status}; value ${item.value}; handled by ${item.owner}${note?`; note: ${note}`:""}`;setHistoryById(history=>({...history,[item.id]:[historyEntry,...(history[item.id]||[])]}));if(status==="Won"){onWon({id:item.id.replace(/^QUO|^OPP/,"PRJ"),customerId:item.customerId,name:item.name,type:"Opportunity conversion",status:"New",progress:"0%",start:new Date().toLocaleDateString(),due:"To be scheduled"});setItems(rows=>rows.filter(row=>row.id!==item.id));setSelected(null);setIsNew(false);announce("Won opportunity transferred to the Project list");return;}setItems(rows=>selected?rows.map(row=>row.id===selected.id?item:row):[item,...rows]);setSelected(null);setIsNew(false);announce(note?"Opportunity saved and note moved to History":"Opportunities saved")};
...
function RecordsPage({module,query,filter,setFilter,announce,onNewContact,onEditCustomer,onDeleteCommercial,onDeleteCustomer,onDeleteEmployee,rowsOverride}:{module:string,query:string,filter:string,setFilter:(x:string)=>void,announce:(x:string)=>void,onNewContact:()=>void,onEditCustomer:(customer:Row)=>void,onDeleteCommercial:(record:Row)=>void,onDeleteCustomer:(customer:Row)=>void,onDeleteEmployee:(employee:Row)=>void,rowsOverride?:Row[]}) {
...
const accessPages = [
  ...
  {name:"Maintain Offers",source:"Opportunities Extended",count:171,icon:"OP",color:"green",groups:[
    ["Inquiry",["OpportunityID","Document Type","Inquiry No / RFQ No","Inquiry Date","Inquiry Submission Date","Submitted","Project Name","Opportunity Type","Year"]],
    ...
    ["History & documents",["Comments","Comment Date","History","Notes","Scope Note","Email","Attachments"]],
  ]},
